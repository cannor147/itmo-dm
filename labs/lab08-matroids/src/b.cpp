...
private:
    ...
    vector<pair<long long, pair<size_t, size_t>>> edges;
...
public:
    ...
    set<pair<long long, pair<size_t, size_t>>> get_mst() {
        set<pair<long long, pair<size_t, size_t>>> result;
        ...
        for (auto &e : edges) {
            if (d.findSet(e.second.first) != d.findSet(e.second.second)) {
                d.unionSets(e.second.first, e.second.second);
                result.insert(e);
            }
        }
        return result;
...
int main() {
    ...
    cin >> n >> m >> s;
    graph g(n);
    vector<pair<pair<long long, pair<size_t, size_t>>, size_t>> edg;
    for (size_t i = 0; i < m; i++) {
        size_t a, b;
        long long c;
        cin >> a >> b >> c;
        g.add_edge(a - 1, b - 1, c);
        edg.push_back({{c, {a - 1, b - 1}}, i + 1});
    }
    auto mst = g.get_mst();
    ...
    for (size_t i = 0; i < m; i++) {
        auto re = edg[i].first;
        swap(re.second.first, re.second.first);
        if (mst.count(edg[i].first) > 0 || mst.count(re) > 0) continue;
        if (count <= s - edg[i].first.first) {
            count += edg[i].first.first;
            result.push_back(edg[i].second);
        } else {
            break;
        }
    }
